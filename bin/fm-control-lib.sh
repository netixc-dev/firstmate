...
# Which named keys a backend adapter can deliver. Every retained session
# provider normalizes Enter, Ctrl+C, and the Ctrl+U composer clear.
...
# herdr implement fm_backend_agent_state, so only these backends can prove an
# agent stopped. The control plane refuses
