...
rules_err=$(jq -r --argjson verified_harnesses "$VERIFIED_HARNESSES" --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
  ...
  elif any((.rules // [])[] | profiles(.use)[]; (verified(.harness) | not)) then
    "each use profile must name a verified harness: " + ([.rules[] | profiles(.use)[] | select(verified(.harness) | not) | .harness] | unique | join(", "))
  ...
  elif has("default") and any(profiles(.default)[]; (verified(.harness) | not)) then
    "each default profile must name a verified harness: " + ([profiles(.default)[] | select(verified(.harness) | not) | .harness] | unique | join(", "))
