...
#
# The test is deliberately NOT a treehouse path prefix. Treehouse's root is
# configurable (--root, TREEHOUSE_ROOT, config, and a relative in-project pool),
# so a prefix check would refuse legitimate roots, accept whatever a mutable env
# var names, and add exactly the policy surface this registration must not grow.
# The structural test is verified for treehouse worktrees, which are linked git
# worktrees.
#
