...
fm_backend_herdr_server_ensure() {  # <session>
  ...
  (
    unset FM_HOME FM_ROOT_OVERRIDE FM_STATE_OVERRIDE FM_DATA_OVERRIDE FM_PROJECTS_OVERRIDE FM_CONFIG_OVERRIDE \
      CURSOR_AGENT CURSOR_INVOKED_AS CLAUDECODE PI_CODING_AGENT FM_PI_HARNESS FM_SUPERVISION_MODEL
...
fm_backend_herdr_normalize_key() {  # <key>
  case "$1" in
    ...
    # C-u clears a composer line; retain the generic backend key mapping.
...
# same commit instead of maintaining backend-local shape patterns.
