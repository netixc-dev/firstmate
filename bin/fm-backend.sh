...
# HERDR_ENV=1 alone (no $TMUX) selects herdr. Other terminal markers are not
...
# always wins. Auto-detected herdr stays silent like tmux.
fm_backend_name() {
  local line v
...
# harness readiness/delivery checks) can ask the same question
