...
#
# Stdin mode extracts .tool_input.command for Claude and Codex.
# Cursor delivers the same .tool_input.command shape with
...
With no --command, reads a PreToolUse-style JSON payload on stdin (Claude/Codex/Cursor tool_input.command).
