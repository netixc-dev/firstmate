...
`state.authStatus` is present only for some providers (observed for the xAI/Pi `grok` provider row), so its absence is missing evidence, not a credential fault.
