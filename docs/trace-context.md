...
Ship and scout spawns reach that site on every spawn backend (`tmux`, `herdr`, `zellij`); a Secondmate reaches it on every backend that accepts a Secondmate spawn (`tmux`, `herdr`, `zellij`).
