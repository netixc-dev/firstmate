...
Legacy `agy` task records remain refused; a legacy `harness=devin`, `harness=rovo`, `harness=muse`, or `harness=grok*` task remains steerable through `fm-send.sh`, but [lifecycle control](agent-control.md#fail-closed-boundaries) cannot manage those removed adapters.
