...
Treehouse remains the worktree provider for tmux and Herdr, since both are session providers.
...
The retired `agy`, Devin, Rovo, Muse, and standalone Grok CLI worker adapters are unsupported: explicit harness selections, static pins, and dispatch profiles naming them are refused before task mutation rather than falling back to another runtime.
Legacy `agy` task records remain refused; a legacy `harness=devin`, `harness=rovo`, `harness=muse`, or `harness=grok*` task remains steerable through `fm-send.sh`.
[Lifecycle control](agent-control.md#fail-closed-boundaries) refuses Devin, Rovo, and Muse records even with an explicit replacement, while a legacy Grok record permits only an explicit relaunch onto a supported adapter after its endpoint is proven agent-free.
An implicit relaunch cannot reconstruct the original command, and no legacy record is ever reinterpreted as Pi.
...
Legacy Muse session bindings and legacy Grok hook pointers, registry tokens, and authentication entries are cleanup-only artifacts: guarded replacement and teardown retire only the exact task's Firstmate-owned sidecars, never vendor files they reference.
Vendor installations and session history remain untouched.
The generic raw-command escape hatch remains caller-owned for Devin, Rovo, and Muse, while a raw command whose first executable is `grok*` is refused even through the supported `env` prefix forms; [`fm-spawn.sh --help`](../bin/fm-spawn.sh) owns the exact parsing and refusal boundary.
...
| Custom provider store | Its configured location variables, such as `CODEX_HOME` or `XDG_CONFIG_HOME`; Firstmate's existing explicit Claude store assignment still applies. |
...
The opted-in resolver has authoritative single-provider mappings for `claude`, `codex`, `kimi`, and `cursor`; every other verified harness must declare `provider` explicitly, including multi-provider `pi`, `pi-signed`, `omp`, and `opencode` and unmapped `gemini`.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux` or `herdr`), `jq` for Herdr's JSON-emitting spawn and liveness paths, and the `treehouse` worktree provider for every retained backend.
