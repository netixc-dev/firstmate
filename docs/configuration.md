...
Legacy `agy` task records remain refused; a legacy `harness=devin` task remains steerable through `fm-send.sh` because its record does not distinguish the old adapter from a caller-owned raw command, but [lifecycle control](agent-control.md#fail-closed-boundaries) cannot manage that removed adapter.
