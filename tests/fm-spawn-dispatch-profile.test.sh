...
test_kimi_selections_refuse_before_mutation() {
  ...
  read_case_record "$rec"
  before=$(git -C "$WT_DIR" status --short)
  probe_value="$CASE_DIR/env-probe-value"
  probe_args="$CASE_DIR/env-probe-args"
  cat > "$FAKEBIN_DIR/kimi" <<'SH'
#!/bin/sh
printf '%s' "$X" > "$FM_ENV_PROBE_VALUE"
printf '%s\n' "$@" > "$FM_ENV_PROBE_ARGS"
SH
  cat > "$FAKEBIN_DIR/custom-agent" <<'SH'
#!/bin/sh
printf '%s' "$FOO" > "$FM_ENV_PROBE_VALUE"
printf '%s\n' "$@" > "$FM_ENV_PROBE_ARGS"
SH
  chmod +x "$FAKEBIN_DIR/kimi" "$FAKEBIN_DIR/custom-agent"
  FM_ENV_PROBE_VALUE="$probe_value" FM_ENV_PROBE_ARGS="$probe_args" PATH="$FAKEBIN_DIR:$PATH" \
    /usr/bin/env -S 'X=a\nb kimi --auto' || fail "real env -S control-escape probe failed"
  [ "$(cat "$probe_value")" = "$(printf 'a\nb')" ] || fail "real env -S split the escaped newline assignment"
  [ "$(cat "$probe_args")" = --auto ] || fail "real env -S did not execute the Kimi-shaped probe"
  FM_ENV_PROBE_VALUE="$probe_value" FM_ENV_PROBE_ARGS="$probe_args" PATH="$FAKEBIN_DIR:$PATH" \
  ...
  for form in flag positional config raw command_raw exec_raw nohup_raw env_command_raw command_env_raw wrapper_chain_raw env_raw env_numeric_assignment_raw env_unset_raw env_chdir_raw env_unset_equals_raw env_ignore_raw env_separator_raw env_cluster_raw env_unknown_raw env_split_raw env_split_numeric_assignment_raw env_split_equals_raw env_split_option_raw env_split_cluster_raw env_split_escape_raw env_split_control_raw secondmate; do
    case "$form" in
      ...
      env_split_control_raw) out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness "env -S 'X=a\\nb kimi --auto'"); rc=$? ;;
    ...
    case "$form" in
      env_unknown_raw)
        ...
        ;;
      env_split_escape_raw|env_split_control_raw)
  ...
  raw="env -S 'FOO=hello\\\"world /opt/bin/custom-agent --flag'"
...
test_devin_adapter_selections_refuse_and_raw_command_survives() {
  ...
  assert_not_contains "$out" "unsupported removed harness" "legacy task record was rejected before recovery validation"
  assert_contains "$out" "endpoint" "legacy task did not reach endpoint validation"
  ...
  assert_not_contains "$out" "unsupported removed harness" "raw relaunch was misclassified as an adapter"
  assert_contains "$out" "endpoint" "raw relaunch did not reach endpoint validation"
  ...
  out=$(cd "$CASE_DIR" && run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" secondmate-devin-home-z3 devin --secondmate); rc=$?
  expect_code 1 "$rc" "bare Devin secondmate selection must refuse without a matching home"
  assert_contains "$out" "unsupported removed harness 'devin'" "bare Devin selection was not rejected by positional resolution"
  sm="$CASE_DIR/devin"
...
test_removed_adapter_inputs_preserve_task() {
  ...
  for form in flag raw leading_flag leading_tab_flag positional positional_raw leading_positional_raw secondmate_raw leading_secondmate_raw static secondmate; do
    ...
    leading_flag) out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness " $removed --prompt-interactive"); rc=$? ;;
    leading_tab_flag) out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness "${tab}${removed} --prompt-interactive"); rc=$? ;;
    ...
    leading_positional_raw) out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" " $removed --prompt-interactive"); rc=$? ;;
    secondmate_raw) out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$removed --prompt-interactive" --secondmate); rc=$? ;;
    leading_secondmate_raw) out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" " $removed --prompt-interactive" --secondmate); rc=$? ;;
    static)
      printf '%s\n' "$removed" > "$HOME_DIR/config/crew-harness"
      out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness codex); rc=$?
      printf '%s\n' codex > "$HOME_DIR/config/crew-harness"
  ...
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" --relaunch --harness " $removed --prompt-interactive"); rc=$?
  expect_code 1 "$rc" "leading whitespace on a direct raw relaunch override must refuse"
  assert_contains "$out" "unsupported removed harness" "leading-whitespace relaunch override did not name the refusal"
  [ "$(cat "$HOME_DIR/state/$id.meta")" = "$meta_before" ] || fail "leading-whitespace relaunch override changed its record"
  [ ! -s "$LAUNCH_LOG" ] || fail "leading-whitespace relaunch override launched another runtime"
...
test_unrelated_raw_argument_keeps_survivor_launch() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$nice_id" "$PROJ_DIR" --harness "nice -n 5 other --prompt $removed"); rc=$?
  expect_code 0 "$rc" "nice must preserve an unrelated leaf's removed-name argument"
...
test_secondmate_home_with_literal_dollar_is_not_a_command() {
  ...
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$sm" --secondmate --harness pi); rc=$?
