...
test_claude_busy_signature_uses_real_capture_shapes() {
  ...
  pane_busy cross claude && fail "Claude must ignore removed adapter footer"
  printf 'Working...\n' > "$composer"
  pane_busy cross codex && fail "Codex must ignore Pi's Working footer"
  printf 'esc interrupt\n' > "$composer"
  pane_busy cross codex && fail "Codex must ignore removed adapter footer"
  printf 'Ctrl+c:cancel\n' > "$composer"
  printf 'esc interrupt\n' > "$composer"
  pane_busy cross pi && fail "Pi must ignore removed adapter footer"
  ...
  printf 'esc interrupt\n' > "$composer"
  pane_busy omp-cross omp && fail "omp must ignore removed adapter footer"
  printf 'Ctrl+c:cancel\n' > "$composer"
