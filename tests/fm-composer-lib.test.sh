...
#   3. The AGENT prompt glyphs `❯` (claude), `›` (codex), and `→`
...
test_stripped_unbordered_content_uses_plain_content() {
  ...
  # Loss of the styled glyph must leave the supported plain-row signal usable.
  for plain in '❯' '›' '→'; do
    out=$(classify 0 '' '' sensitive "$plain")
    [ "$out" = empty ] \
...
test_agent_glyphs_are_empty_bordered_and_bare() {
  ...
  out=$(classify 0 '⟩'); [ "$out" != empty ] || fail "retired bare glyph must not prove empty"
  out=$(classify 0 '' '' sensitive '⟩'); [ "$out" = unknown ] || fail "retired stripped glyph must stay unknown"
  pass "fm_composer_classify_content: only supported agent glyphs prove empty"
}
...
#
# Fixtures retain the audit's byte-level captures of supported idle harnesses:
...
# (styled+cursor+identity), a styled capture without identity, and an unstyled
# capture. Every
...
CAPS_STYLED_NOID=$'styled=1\ncursor=0\nidentity=0\nrows=20' # generic styled capture
CAPS_UNSTYLED=$'styled=0\ncursor=0\nidentity=0\nrows=20'       # generic unstyled capture
...
test_matrix_claude_bare_nbsp_row() {
  ...
  assert_screen "claude typed on plain backends" unknown "$CAPS_UNSTYLED" "$typed"
...
test_matrix_claude_arrow_statusline_footer() {
  ...
  assert_screen "claude idle under an arrow statusline on styled capture" empty "$CAPS_STYLED_NOID" "$screen"
...
test_composer_footer_zone_is_shape_independent() {
  ...
  assert_screen "boxed claude idle under an arrow statusline on styled capture" empty "$CAPS_STYLED_NOID" "$screen"
  ...
  assert_screen "unproven banner still yields to the bare row below it" empty "$CAPS_UNSTYLED" "$screen"
...
test_matrix_codex_dim_hint_row() {
  ...
  assert_screen "codex idle on plain backends" unknown "$CAPS_UNSTYLED" "$plain"
...
test_matrix_cursor_reverse_video_placeholder_remnant() {
  ...
  out=$(fm_composer_classify_screen "$CAPS_UNSTYLED" "$typed_plain")
  [ "$out" != empty ] \
...
❯ fix the flaky test
 π  · ◔ GPT-6-Astra · 🌳 …-workspace · ⑂ detached · ◫ 15.4%/272K ⟲ · (sub)'
  ...
  assert_screen "idle omp on a plain capture" empty "$CAPS_UNSTYLED" "$idle_unicode"
...
test_matrix_codex_idle_starfield_furniture() {
  ...
  assert_screen "codex 0.154 idle on styled capture" empty "$CAPS_STYLED_NOID" "$screen"
...
test_matrix_pi_separated_needs_identity() {
  ...
  assert_screen "pi idle on styled capture without identity" unknown "$CAPS_STYLED_NOID" "$screen"
  ...
  assert_screen "pi pair without identity capability" unknown "$CAPS_UNSTYLED" "$screen"
  ...
  assert_screen "lone glyph on plain backend" empty "$CAPS_UNSTYLED" "$typed"
...
test_matrix_grok_titled_bottom_border() {
  ...
  assert_screen "grok placeholder on plain backends" empty "$CAPS_UNSTYLED" "$placeholder_draft"
...
test_matrix_bordered_shell_glyph_box() {
  # A shell-glyph composer remains a generic bordered shape on every backend.
  local screen
  screen=$'╭────────────────────────╮\n│ >                      │\n╰────────────────────────╯'
  assert_screen "shell glyph on tmux" empty "$CAPS_TMUX" "$screen" 1
  assert_screen "shell glyph on herdr" empty "$CAPS_STYLED" "$screen"
  assert_screen "shell glyph on styled capture" empty "$CAPS_STYLED_NOID" "$screen"
  pass "matrix: bordered shell-glyph box reads empty through the shared owner"
}

test_matrix_claude_styled_ansi_dump() {
  # Real claude captured through a styled ANSI dump (capability established by
  # the audit): `ESC[m` `❯` U+00A0.
  local screen plain
  screen=$'styled pane transcript\n'"${ESC}[m❯${NBSP}"
  plain=$'styled pane transcript\n❯'"$NBSP"
  assert_screen "claude styled dump on tmux" empty "$CAPS_TMUX" "$screen" 1
  assert_screen "claude styled dump on herdr" empty "$CAPS_STYLED" "$screen"
  assert_screen "claude styled dump without identity" empty "$CAPS_STYLED_NOID" "$screen"
  assert_screen "claude styled dump on plain backends" empty "$CAPS_UNSTYLED" "$plain"
  pass "matrix: the real claude styled ANSI dump reads empty in both locales"
}
...
test_strict_blank_row_divergence() {
  ...
  out=$(fm_composer_classify_screen "$CAPS_UNSTYLED" $'\n\n')
...
test_contiguous_transcript_reanchors_on_live_prompt() {
  ...
  assert_screen "contiguous transcript live prompt on cursorless plain backend" empty "$CAPS_UNSTYLED" "$screen"
...
test_cursorless_bare_wrap_region_classifies() {
  ...
  [ "$out" = empty ] || fail "cursorless ghost wrap on a styled capture should be empty, got '$out'"
...
test_cursorless_container_rejects_contiguous_lower_activity() {
  local box leftbar grok shell_box opencode
  box=$'╭────────────────────────╮\n│ ❯                      │\n╰────────────────────────╯\nWorking on request...'
  ...
  assert_screen "blank-separated shell-glyph footer" empty "$CAPS_UNSTYLED" "$shell_box"
...
test_bottom_most_candidate_wins() {
  ...
  local screen out
  screen=$'╭────────────────────────╮\n│ permissions: YOLO mode │\n╰────────────────────────╯\n❯'"$NBSP"
  assert_screen "banner above live claude row" empty "$CAPS_UNSTYLED" "$screen"
  out=$(fm_composer_classify_screen "$CAPS_UNSTYLED" $'╭────────────────────────╮\n│ permissions: YOLO mode │\n╰────────────────────────╯\n› Use /skills to list available skills')
...
test_incomplete_lower_box_invalidates_stale_candidate() {
  local screen out
  screen=$'╭────────────────────────╮\n│ ❯                      │\n╰────────────────────────╯\nstartup complete\n╭────────────────────────╮\n│ ❯ clipped live draft  '
  out=$(fm_composer_classify_screen "$CAPS_UNSTYLED" "$screen")
  [ "$out" = unknown ] \
