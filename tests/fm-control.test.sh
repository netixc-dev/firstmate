...
test_harness_family_resolution() {
  ...
  for pair in claude:claude claude-latest:claude codex:codex codex-cli:codex \
      opencode:opencode kimi:kimi cursor:cursor \
...
test_prefixed_recorded_harness_reaches_each_control_verb() {
  ...
  add_task "$dir" t1 opencode-2
  alive_as "$dir" opencode-2
  out=$(run_control "$dir" t1 interrupt); rc=$?
  expect_code 0 "$rc" "interrupt should resolve a prefixed recorded harness"$'\n'"$out"
  [ "$(keys_sent "$dir")" = $'Escape\nEscape' ] \
    || fail "an opencode-prefixed task should receive OpenCode's two interrupt keys"
  ...
  [ "$(literals "$dir")" = /exit ] \
    || fail "a opencode-prefixed task should receive OpenCode's exit command"
...
test_unverified_harness_is_refused() {
  ...
  for removed in "$(printf 'a%s' gy)" devin rovo rovo-wrapper muse muse-bin-0.1.0 muse-wrapper grok grok-2; do
    ...
    for verb in interrupt exit relaunch; do
      ...
      case "$removed" in
        grok|grok-2) assert_contains "$out" "unsupported legacy Grok record" "refusal should name the removed adapter: $removed" ;;
...
test_harness_kind_capability() {
  ...
  for harness in claude codex opencode pi pi-signed kimi omp; do
